 ...
 * 7. Reliability score (attendance history)
 *
...
const WEIGHTS = {
  district: 32,        // Must be in same district
  category: 22,        // Skill match (nurse vs attendant)
  availability: 14,    // Immediate availability
  shift: 9,            // Day/Night/24hr compatibility
  experience: 9,       // More experience = higher score
  gender: 4,           // Patient preference for gender
  reliability: 10,     // Attendance/history reliability (0-100 from DB)
};
...
function scoreReliability(staff: Staff): number {
  // No attendance data = no reliability points
  if (!staff.reliability_score || staff.reliability_score === 0) return 0;

  // Scale: reliability_score (0-100) maps to 0-RELIABILITY_WEIGHT points
  return Math.round((staff.reliability_score / 100) * WEIGHTS.reliability);
}
...
export function matchStaffToPatient(
  ...
  const results: MatchResult[] = candidates
    .map(staff => {
      ...
      const reliabilityScore = scoreReliability(staff);

      const totalScore = districtScore + categoryScore + availabilityScore +
                         shiftScore + experienceScore + genderScore + reliabilityScore;
      ...
      if (reliabilityScore >= WEIGHTS.reliability * 0.8) reasons.push(`Highly reliable (${staff.reliability_score}%)`);
      else if (reliabilityScore > 0) reasons.push(`Moderately reliable (${staff.reliability_score}%)`);
