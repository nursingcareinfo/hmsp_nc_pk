...
export const dataService = {
  ...
  addPatient: async (patient: Omit<Patient, 'id'>): Promise<Patient> => {
    if (supabase) {
      // Strip id field if present (auto-generated by trigger)
      ...
      if (error) {
        console.error('Supabase patient insert error:', error.message, error.details);
        throw new Error(`Failed to save patient: ${error.message}`);
      }
      if (data) return data as Patient;
    }

    // Fallback: localStorage only if no Supabase configured
