...
export const StaffMatchingModal: React.FC<StaffMatchingModalProps> = ({
  ...
  const handleAssign = async (match: MatchResult) => {
    try {
      // Update patient with assigned staff in Supabase
      await dataService.updatePatient(patient.id, {
        assigned_staff_id: match.staff.id,
      });

      // Notify parent to update local state (parent handles state update + toast)
      onAssign(match.staff.id);

      // Wait for parent state update to complete before closing modal
      // This prevents the race condition where onClose() fires before
      // the detail panel's selectedPatient state is updated
